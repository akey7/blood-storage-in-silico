...
c_means_dfs, wide_timeseries_dfs, fuzzy_objectives_df = c_means_metabolite_trajectories(df)
c_means_filename = joinpath("output", "c_means_clusters.csv")
CSV.write(c_means_filename, c_means_dfs[end])
println("Wrote $c_means_filename")
plot_c_means_for_all_additives(c_means_dfs[end], wide_timeseries_dfs[end])
